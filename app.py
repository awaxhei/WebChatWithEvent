...
class ChatDB:
    ...
    def _migrate_db(self):
        with self._get_conn() as conn:
            try: conn.execute("ALTER TABLE events ADD COLUMN story_time TEXT DEFAULT ''")
            except sqlite3.OperationalError: pass
            try: conn.execute("ALTER TABLE conversations ADD COLUMN events_initialized INTEGER DEFAULT 0")
            except sqlite3.OperationalError: pass

    def create_conversation(self):
        conv_id = uuid.uuid4().hex[:12]
        with self._get_conn() as conn: conn.execute("INSERT INTO conversations(id) VALUES(?)", (conv_id,))
        return conv_id
...
class DeepSeekClient:
    ...
    def chat_stream(self, messages, system_prompt=""):
        ...
        try:
            ...
            first_content = False; prefix_buf = ""
                ...
                if line.startswith("data: "):
                    ...
                    try:
                        ...
                        if not first_content:
                            prefix_buf += content; stripped = prefix_buf
                            for p in prefixes:
                                if prefix_buf.startswith(p): stripped = prefix_buf[len(p):].strip(); break
                            if len(prefix_buf) >= max(len(p) for p in prefixes) or stripped != prefix_buf:
                                first_content = True
            ...
            if not first_content and prefix_buf:
                for p in prefixes:
                    if prefix_buf.startswith(p): prefix_buf = prefix_buf[len(p):].strip(); break
                if prefix_buf: yield prefix_buf
        except requests.exceptions.ConnectionError: yield "无法连接到 DeepSeek API，请检查网络。"
        except requests.exceptions.HTTPError as e:
            try: detail = resp.json(); msg = detail.get("error", {}).get("message", str(e))
...
def verify_token(token):
    try: jwt.decode(token, JWT_SECRET_KEY, algorithms=["HS256"]); return True
    except (jwt.ExpiredSignatureError, jwt.InvalidTokenError): return False
...
def _build_event_context(conv_id):
    ...
    if events:
        ...
        if active_events:
            ...
            for e in active_events[-5:]: lines.append(f"  → {e['title']}（{e['description']}）")
        if pending_events:
            lines.append("即将发生/待回应的事件：")
            for e in pending_events[-3:]: lines.append(f"  ⏳ {e['title']}（{e['description']}）")
        if completed_events:
            lines.append("近期已完成的事件：")
            for e in completed_events[-3:]: lines.append(f"  ✓ {e['title']}")
    return "\n".join(lines)
...
def auth_login():
    ...
    password = data.get("password", ""); remember = data.get("remember", False)
...
def chat():
    ...
    if not conv_id: conv_id = db.create_conversation()
        ...
        if conv_id not in [c["id"] for c in conversations]: conv_id = db.create_conversation()
    ...
    def generate():
        ...
        if should_run_event_ai:
            ...
            if event_result:
                # 先推送随机事件（世界旁白），紧接着推送事件数据
                random_event = event_result.get("random_event")
                if random_event and isinstance(random_event, dict):
                    yield f"data: {json.dumps({'type': 'world_narration', 'content': random_event.get('description', ''), 'intensity': random_event.get('intensity', 'low')})}\n\n"
                ...
                yield f"data: {json.dumps({'type': 'event_update', 'events': all_events, 'has_push': event_result.get('should_push', False), 'push_hint': event_result.get('push_hint', '')})}\n\n"
...
@require_auth
def get_history(): return jsonify(db.get_conversations())
...
@require_auth
def new_conversation(): return jsonify({"id": db.create_conversation()})
